/**
 * PixelGameMaker – main.js (v0.5.1 patched, portable + full APIs + topbar cleanup)
...
for (const d of [BASE_DIR, ASSETS_DIR, MODULES_DIR, UPDATES_DIR]) {
...
ipcMain.handle("assets:list", async (_evt, rel = "") => {
  try {
    ...
    return { ok: true, cwd: slashify(path.relative(root, abs)), items };
  } catch (e) {
...
ipcMain.handle("assets:pick", async (_evt, opts = {}) => {
  ...
  if (res.canceled || !res.filePaths?.length) return { ok: false, canceled: true };
  const rels = res.filePaths.filter(p => within(root, p)).map(p => slashify(path.relative(root, p)));
...
ipcMain.handle("assets:cwdUp", async (_evt, rel = "") => {
  ...
  return { ok: true, path: slashify(path.relative(root, parent)) };
});
...
ipcMain.handle("assets:upload", async (_evt, cwdRel = "", srcFile = "") => {
  try {
    ...
    return { ok: true, rel: slashify(path.relative(root, dest)) };
  } catch (e) {
    return { ok: false, message: String(e?.message || e) };
  }
});
...
ipcMain.handle("modules:addViaPicker", async () => {
  ...
  return {
    ok: true,
    module: { id: path.basename(dest), name: path.basename(dest, ext), path: dest, url: toFileUrl(dest), size: st.size, mtime: st.mtimeMs },
  };
});
...
ipcMain.handle("updates:activate", async (_evt, id) => {
  // Minimal: only record active ID (no hotpatching here).
